...
async def scan_port(target, port, timeout):
    try:
        # Attempt to open an asynchronous TCP connection to the target port
        reader, writer = await asyncio.wait_for(
            # Records timeout as constant in relation to target and port number while async scan occurs
            asyncio.open_connection(target, port), timeout=timeout
        )
        # Special handling for RDP (port 3389): mark as open without requesting further data
        if port == 3389:
            # Immediately closes the connection without attempting to read scanned banner
            writer.close()
            await writer.wait_closed()
            # Retrieves the service name from port mapping
            service_name = get_service_name(port)
            return {
                'Port': port,
                'Service': service_name,
                'Status': 'Open',
                'Banner': 'No banner (RDP special case)'
            }
        # TODO:     Port range greater than between 1 to 5000 will continually error out
        #           unless I keep this weirdly redundant special case for Port 3389; will
        #           probably stay unless I can figure out a workaround that doesn't result
        #           in spending a concerning amount of time on this one problem.

        # For other ports, attempt to retrieve the banner
        banner = ''
        try:
        ...
        # Checks for unnecessary tags or metadata and removes them
        if banner:
        ...
        # Ignore common errors like timeouts and connection refusals
        return None
    except Exception as e:
        # Capture unexpected errors with port information
        return {'Port': port, 'Error': f"Unexpected error: {e}"}

#   VAR 05:         port_scan
#   DESCRIPTION:    Port range loopback for port connectivity
async def port_scan(target, start_port, end_port, timeout, batch_size=200):
    # Begins runtime
...
async def scan_network(target_range, start_port, end_port, timeout):
    try:
        ...
        for ip in network.hosts():
            print(f"\nStarting scan on {ip}")
            await port_scan(str(ip), start_port, end_port, timeout)
